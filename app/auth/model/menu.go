...
// 获取菜单有权限的操作列表
func (Menu) GetMenuButton(cdb *gorm.DB, adminsid uint64, menuCode string, btns *[]string) (err error) {
	...
	err = cdb.Raw(sql, menuCode, adminsid).Pluck("operate_type", btns).Error
	return
}

// 获取管理员权限下所有菜单
func (Menu) GetMenuByAdminsid(cdb *gorm.DB, adminsid uint64, menus *[]Menu) (err error) {
	sql := `select * from tb_sys_menu
	      where id in (
					select menu_id from tb_sys_role_menu where 
				  role_id in (select role_id from tb_sys_admins_role where admins_id=?)
				)`
	err = cdb.Raw(sql, adminsid).Find(menus).Error
	return
}

// 删除菜单及关联数据
func (Menu) Delete(cdb *gorm.DB, menuids []uint64) error {
	tx := cdb.Begin()
	defer func() {
		if r := recover(); r != nil {
			tx.Rollback()
...
func deleteMenuRecurve(cdb *gorm.DB, parentID uint64) error {
	where := &Menu{}
	where.ParentID = parentID
	var menus []Menu
	dbslect := cdb.Where(&where)
	if err := dbslect.Find(&menus).Error; err != nil {
		return err
	}
	for _, menu := range menus {
		if err := cdb.Where("menu_id = ?", menu.ID).Delete(&RoleMenu{}).Error; err != nil {
			return err
		}
		if err := deleteMenuRecurve(cdb, menu.ID); err != nil {
			return err
		}
	}
